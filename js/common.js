//获取地理位置，与web端同步
function getRightLocation(Lat,Lon,id){	//纬度，经度，id
	$.ajax({
...
// H5 plus事件处理
var ws=null,as='pop-in',quits=null;
function plusReady(){
	if(ws){return;}
	ws=plus.webview.currentWebview();
	// Android处理返回键
	plus.key.addEventListener('backbutton',function(){
		if("screenPage/mainPage.html" == plus.webview.currentWebview().id){
 ...
 */
w.clicked=function(id,wa,ns,ex,ws){
	if(openw){//避免多次打开同一个页面
		return null;
	}
	ex||(ex=as);
	if(w.plus){
		var idWeb = plus.webview.getWebviewById(id);
		if(idWeb){
			idWeb.show(ex);
			return;
		}
...
// 兼容性样式调整
var adjust=false;
function compatibleAdjust(state){
	if(!state&&"screenPage/mainPage.html" == plus.webview.currentWebview().id){
